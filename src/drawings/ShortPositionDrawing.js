...
export class ShortPositionDrawing extends DrawingBase {
    ...
    clone() {
        const c = new ShortPositionDrawing(this.anchors[0].time, this.anchors[0].price);
    ...
    updatePendingAnchor(time, price) {
        ...
        const diff = this.anchors[0].price - price;
    ...
    draw(ctx, timeScale, priceScale) {
        ...
        // For short: profit zone is below entry (target below), loss zone is above entry (stop above)
        ...
        const targetPct = Math.abs((entry - target) / entry * 100).toFixed(3);
        const stopPct = Math.abs((stop - entry) / entry * 100).toFixed(3);
        const targetAmount = Math.round(Math.abs(entry - target) * this.qty);
        const stopAmount = Math.round(Math.abs(stop - entry) * this.qty);
        const riskReward = stopAmount > 0 ? (targetAmount / stopAmount).toFixed(1) : '---';
        const pnl = Math.round((entry - target) * this.qty);

        ctx.font = '11px sans-serif';
        ctx.textBaseline = 'middle';
        ctx.textAlign = 'center';

        // Target below entry for short → label below target line
        ...
        // Stop above entry for short → label above stop line
